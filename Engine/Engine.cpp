...
void Engine::Run()
{
	...
	thread printFrame([this]() {
		while (!gameOver)
		{
			if (!settingsOpen && !calculateFrame) { WriteConsoleOutput(console, screen, bufferSize, { 0,0 }, &windowSize); }
		}});
	thread infoKey([this]() {
		while (!gameOver) {
			if (_kbhit()) {
	...
	thread renderImage([this]() {
		while (!gameOver) {
			/*if (timeInSeconds * 1000 < 16)
			{
	...
	thread checkPlayerInTeleport([this]() {
		while (!gameOver) { checkPlayerInTeleport(); }});
	thread motionPlayer([this]() {
		while (!gameOver) {
			//this_thread::sleep_for(chrono::milliseconds(16));
			player.motion(mapInfo.map, mapInfo.mapSizeHorizontal, 10);
		}
		});
	if (gameOver)
	{
